...
달리 바쁘냥은 **CPU뿐 아니라 GPU 연산 부하도 함께** 봅니다 — ML 학습·임베딩 같은
무거운 GPU 연산 작업에도 고양이가 빨라집니다.

🇺🇸 [English README](README.md)

<p align="center">
  <a href="https://github.com/mangomandu/busycat/releases/latest/download/BusyCat-1.1.4-macOS.dmg"><strong>Apple Silicon Mac용 다운로드 (.dmg)</strong></a>
  <br>
...
- **CPU와 GPU 연산 부하**를 함께 감시. 속도 기준 선택: 가장 바쁜 쪽(CPU·GPU 연산) / CPU / GPU 연산 /
  메모리.
- **상세 패널**(고양이 클릭): CPU · GPU · 메모리 · 디스크 · 열 상태 · 네트워크 · 배터리 —
  가능한 항목은 활성 상태 보기(Activity Monitor)와 비교 가능한 기준으로 계산.
- **온도 상세 hover**: 최고 온도 센서, macOS 열 압박 단계, `pmset` 속도 제한, 상위
  온도 센서를 나눠 표시.
- 메뉴바 텍스트 선택: 표시 안 함 / 고양이 속도 % / CPU % / GPU 연산 % / 메모리 % /
  온도 / 열 압박.
- macOS의 실제 메모리 압력 단계(정상/주의/위험)에 따라 고양이 옆에 작은 **생선 더미**를 표시하는 옵션.
...
현재 배포본은 **Apple Silicon(M1 이상), macOS 13 Ventura 이상 전용**입니다. Intel Mac은
지원하지 않습니다.
...
Homebrew에서 신뢰되지 않은 tap이라는 오류가 나오면 한 번만 신뢰 등록한 뒤 다시
설치하세요.
...
개발자나 터미널에 익숙한 사용자는 소스에서 직접 빌드할 수도 있습니다. 인터넷에서
받은 앱을 실행하는 방식이 아니라 내 Mac에서 직접 만든 앱이라, 다운로드 앱에 붙는
macOS 보안 경고를 피하는 데 도움이 됩니다.
...
이 방법은 Xcode Command Line Tools가 필요합니다.
...
- **메모리 / 디스크 / 네트워크 / 배터리 / 열 상태**: `vm_statistics64`, 볼륨 용량,
  `NET_RT_IFLIST2` 64비트 바이트 델타, IOKit `AppleSmartBattery`, `ProcessInfo.thermalState`,
  IOHID/AppleSMC 온도 센서, `pmset -g therm` — 각각 가능한 무권한 경로를 우선 사용.
- **메모리 압력**: 퍼센트를 추정하지 않고 macOS의 공개 Dispatch 메모리 압력 이벤트에서
  정상/주의/위험 상태를 직접 받습니다.
...
- **샘플링 최적화**: 첫 상세 정보는 앱 시작 뒤 전용 백그라운드 큐에서 미리 읽어
  캐시합니다. 이후 메뉴가 닫혀 있을 때는 고양이 속도에 필요한 CPU/GPU 연산/메모리만
  읽고, 상세 센서는 메뉴가 열렸을 때 갱신합니다. 느리게 변하는 값은 약 5초 동안
  캐시하며 `pmset -g therm`은 더
  무거워서 내부적으로 약 30초 캐시를 둡니다.
- **정확도 한계**: GPU 연산 부하는 Apple Silicon의 IOKit 카운터를 최대한 일관되게
