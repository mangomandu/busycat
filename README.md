...
## 빌드 / 실행 (구현됨)
```bash
./make_app.sh          # swift build -c release → RuncatGPU.app 생성 (애드혹 서명)
open RuncatGPU.app     # 메뉴바에 고양이 등장 (Dock 아이콘 없음 = LSUIElement)
# 설치: cp -R RuncatGPU.app /Applications/ 후 한 번 open
# 종료: 메뉴바 고양이 클릭 → Quit RuncatGPU (⌘Q)
```
...
## 구현 메모 (코드 위치)
- `Sources/RuncatGPU/UsageReader.swift` — `SystemSampler`: CPU(HOST_CPU_LOAD_INFO 델타)·GPU(IOKit, `max(Device,Renderer,Tiler)`)·메모리(HOST_VM_INFO64)·디스크(volume capacity)·네트워크(getifaddrs 바이트 델타)·배터리(IOKit.ps).
- `Sources/RuncatGPU/CatFrames.swift` — **RunCat 고양이 5프레임을 base64 PNG로 내장**(자체 완결, 경로 의존 없음). 원본 아트: `Kyome22/menubar_runcat` (Apache 2.0). 템플릿 이미지라 다크/라이트 자동.
- `Sources/RuncatGPU/AppDelegate.swift` — NSStatusItem, 1초마다 샘플링 → 속도 갱신(RunCat 공식), 프레임 타이머(.common 모드).
- 디버그: `RuncatGPU --dump <dir>`(프레임 PNG 덤프), `RuncatGPU --metrics`(지표 1회 출력).
...
## 남은 옵션 (원하면)
- **로그인 시 자동 실행:** 시스템 설정 → 일반 → 로그인 항목에 `RuncatGPU.app` 추가 (또는 `SMAppService`로 메뉴 토글 추가 가능).
